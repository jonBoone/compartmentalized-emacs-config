;;;; compartmentalized-writing-config.el --- Configuration for writing text documents  -*- lexical-binding: t; -*-
...
;; Zettelkasten systems, those are found in the `compartmentalized-org-config'
;; module.
...

;;; Whitespace
(defun compartmentalized-writing-configure-whitespace (use-tabs &optional use-globally &rest enabled-modes)
...
(compartmentalized-writing-configure-whitespace nil t)

;; overwrites the above to turn to use tabs instead of spaces,
;; does not turn off global-whitespace-mode, adds a hook to
;; makefile-mode-hook
(compartmentalized-writing-configure-whitespace t nil 'makefile-mode)
...
(compartmentalized-writing-configure-whitespace nil)
...
;; and derived modes.
(compartmentalized-writing-configure-whitespace nil nil 'prog-mode)"
  (if use-tabs
...
(defun compartmentalized-latex-use-pdf-tools ()
...
(defun compartmentalized-writing-tex-warning-if-no-latex-executable ()
...
(add-hook 'tex-mode-hook #'compartmentalized-writing-tex-warning-if-no-latex-executable)
...
(when (and (executable-find "latex")
  ...
  (with-eval-after-load 'latex
    (when (require 'auctex-latexmk nil 'noerror)
      ...
      (defun compartmentalized-writing-tex-make-latexmk-default-command ()
        "Set `TeX-command-default' to \"LatexMk\"."
        (setq TeX-command-default "LatexMk"))
      (add-hook 'TeX-mode-hook #'compartmentalized-writing-tex-make-latexmk-default-command))))
...
(when (locate-library "pdf-tools")
  ...
  (defun compartmentalized-writing-load-pdf-tools ()
    "Attempts to require pdf-tools, but for attaching to hooks."
    (require 'pdf-tools nil :noerror))
  (add-hook 'doc-view-mode-hook #'compartmentalized-writing-load-pdf-tools)

  ;; when pdf-tools is loaded, apply settings.
  (with-eval-after-load 'pdf-tools
    (setq-default pdf-view-display-size 'fit-width)))

(provide 'compartmentalized-writing-config)
;;; compartmentalized-writing-config.el ends here
