...
(when (require 'vertico nil :noerror)
  ...
  ;; they have been turned on by compartmentalized-defaults-config, because they interfere
  ;; with this module.
  (with-eval-after-load 'compartmentalized-defaults-config
    (fido-mode -1)
...
(when (require 'cape nil :noerror)
  ...
  (defun compartmentalized-completion-corfu-eshell ()
    "Special settings for when using corfu with eshell."
    (setq-local corfu-quit-at-boundary t
                corfu-quit-no-match t
                corfu-auto nil)
    (corfu-mode))
  (add-hook 'eshell-mode-hook #'compartmentalized-completion-corfu-eshell))

(provide 'compartmentalized-completion-config)
;;; compartmentalized-completion.el ends here
