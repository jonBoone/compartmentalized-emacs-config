;;; compartmentalized-ide-config.el --- Provide IDE-like features -*- lexical-binding: t; -*-
...
;;; Eglot
(defun compartmentalized-ide--add-eglot-hooks (mode-list)
...
`(add-hook 'some-mode-hook #'eglot-ensure)'"
  (dolist (mode-def mode-list)
    (let ((mode (if (listp mode-def) (car mode-def) mode-def)))
      (cond
       ((listp mode) (compartmentalized-ide--add-eglot-hooks mode))
       (t
...
(defun compartmentalized-ide--lsp-bin-exists-p (mode-def)
...
(defun compartmentalized-ide-eglot-auto-ensure-all ()
...
binary is detected on the system."
  (when (require 'eglot nil :noerror)
    (compartmentalized-ide--add-eglot-hooks (seq-filter
                                   #'compartmentalized-ide--lsp-bin-exists-p
...
(defun compartmentalized-ide-configure-tree-sitter (&optional opt-in-only)
...
installed."
  (if (version< emacs-version "29")
      (compartmentalized-ide--configure-tree-sitter-pre-29)
    (compartmentalized-ide--configure-tree-sitter opt-in-only)))
...
(when (require 'ibuffer-project nil :noerror)
  (defun compartmentalized-ide-enhance-ibuffer-with-ibuffer-project ()
    "Set up integration for `ibuffer' with `ibuffer-project'."
    (setq ibuffer-filter-groups (ibuffer-project-generate-filter-groups))
    (unless (eq ibuffer-sorting-mode 'project-file-relative)
      (ibuffer-do-sort-by-project-file-relative)))
  (add-hook 'ibuffer-hook #'compartmentalized-ide-enhance-ibuffer-with-ibuffer-project))

(provide 'compartmentalized-ide-config)
;;; compartmentalized-ide-config.el ends here
